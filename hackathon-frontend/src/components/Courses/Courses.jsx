      ...
      if (newcourse.playlist !== undefined) {
        ...
        while (lecturestack.length > 0) {
          ...
          if (currlec.playlist !== undefined) {
            ...
            for (var j = lastVideoInLayer; j > -1; j--) {
              lecturestack.push(currlec.playlist[j]);
            }
          }
        }
      ...
      <div className='courses'>
        ...
        {courses.err === 0 ? (
          courses.courses.length === 0 ? (
            <Typography>
              It looks like you're not in any courses... why not{" "}
              <a href='search'>try a few?</a>
            </Typography>
          ) : (
            courses.courses.map((course) => (
              <CourseCard
                key={course._id}
                onSelectCourse={onSetCurrLecture}
                courseInfo={course}
              />
            ))
          )
          ...
          <Typography color='error'>
            It looks like there was a problem. Are you logged in?
          </Typography>
        )}
      </div>
          ...
          <div>
            <div className='currlecture'>
              <iframe
                ...
                src={
                  ...
                  (currLecture.playlist[currLecture.curr].snippet
                    .resourceId !== undefined
                    ? currLecture.playlist[currLecture.curr].snippet.resourceId
                        .videoId
                    : currLecture.playlist[currLecture.curr].id.videoId)
                }
            ...
            <div className='lecturescroll'>
              {courseContent.map((video) => (
                <LectureCard key={video.id} lectureInfo={video} />
              ))}
            </div>
            <p className='debug'>{JSON.stringify(course)}</p>
          </div>
        ) : (
          <Typography className='course' variant='h6'>
            Choose a Course to continue above to get started
          </Typography>
        )}
      </div>
    </div>
